...
class Embeddings(): 
    ...
    def getEmbeddingBatchRaw(self, start_row: int, batch_size: int): 
        cursor = self.conn.cursor()
        cursor.execute(f"SELECT embedding FROM {self.table_name} WHERE id >= ? LIMIT ?", (start_row, batch_size))
    ...
    def getAllEmbeddings(self, max = 0) -> list[list[int]]:
        cursor = self.conn.cursor()

        cursor.execute(f'SELECT embedding FROM {self.table_name} ORDER BY RANDOM() {f"LIMIT {max}" if max != 0 else ""}')

        all = [pickle.loads(e[0]) for e in cursor.fetchall()]
